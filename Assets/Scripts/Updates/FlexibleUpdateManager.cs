...
{/*
    public class FlexibleUpdateManager : MonoBehaviour
    {
        ...
        private void TryToChangeScene(string reason)
        {
            ...
            {
                Debug.LogWarning("[FlexibleUpdate] TryToChangeScene - CheckForUpdates called again, but already _hasTriedToChangeScene once.");  
                return;
            }
            ...
            Debug.Log("[FlexibleUpdate] TryToChangeScene invoked.");

            if (!isTesting)
            {
                Debug.Log("[FlexibleUpdate] TryToChangeScene - starting CheckForUpdates().");
        ...
        {
            Debug.Log("[FlexibleUpdate] CheckForUpdates started.");
            var appUpdateInfo = updateManager.GetAppUpdateInfo();
            /*
            const float INFO_TIMEOUT = 12f;
            float t = 0f;
            while (!appUpdateInfo.IsDone && t < INFO_TIMEOUT)
            {
                t += Time.unscaledDeltaTime;
                yield return null;
            }
            */
   /*         if (!appUpdateInfo.IsDone || !appUpdateInfo.IsSuccessful)
            {
                Debug.LogWarning($"[FlexibleUpdate] GetAppUpdateInfo failed/timeout. done={appUpdateInfo.IsDone} ok={appUpdateInfo.IsSuccessful}");
                yield break;
            }
            ...
            Debug.Log($"[FlexibleUpdate] availability={updateInfo.UpdateAvailability}");
            Debug.Log($"[FlexibleUpdate] allowed(FLEX)={updateInfo.IsUpdateTypeAllowed(flexible)}");

            if (updateInfo.UpdateAvailability == UpdateAvailability.DeveloperTriggeredUpdateInProgress)
            {
                // Start download in background
                StartCoroutine(StartOrResumeDownload(updateInfo, flexible, isResume: true));
                yield break;
            }

            if (updateInfo.UpdateAvailability == UpdateAvailability.UpdateAvailable &&
                updateInfo.IsUpdateTypeAllowed(flexible))
            {
                // Start download in background
                StartCoroutine(StartOrResumeDownload(updateInfo, flexible, isResume: false));
                yield break;
            }
