...
public class PopUpTextUI : MonoBehaviour
{
    ...
    [SerializeField] private Color nukeColour;
    ...
    private void MissileText(int missileAmt, string missileType)
    {
        GameObject prefab = Instantiate(popUpTextPrefab, transform.position, Quaternion.identity);
        TextMesh textComponent = prefab.GetComponentInChildren<TextMesh>();
        if (!textComponent) return;
        ...
        if (textRenderer)
        {
            ...
            if (missileType == "Shield")
            {
                textRenderer.material.color = shieldColour;
            }
            else if (missileType == "Nuke")
            {
                textRenderer.material.color = nukeColour;
            }
        }
    }
